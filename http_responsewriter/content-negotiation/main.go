// 有时服务器应用程序需要在同一 URI 上提供资源的不同表示形式。
// 服务器会 根据客服端 Header 的 Accept（application/json 或者 text/xml 或者 application/xml ） 响应不同格式的内容
...
func newApp() *iris.Application {
	...
	app.Get("/resource", func(ctx iris.Context) {
		...
		ctx.Negotiation().JSON().XML() // 仅响应 Accept:application/json 或者 text/xml 或者 application/xml ，不会响应 text/html，如果Accept未指定或者为*/* 则按最后一个格式返回
		// Accept-Charset vs:
	...
	app.Get("/resource2", func(ctx iris.Context) {
		...
		ctx.Negotiation().
			JSON(jsonAndXML).
			XML(jsonAndXML).
			HTML("<h1>Test Name</h1><h2>Age 26</h2>") // 这是推荐的方式
	...
	app.Get("/resource3", func(ctx iris.Context) {
		...
		// resources on send.
		ctx.Negotiation().JSON().XML().HTML().EncodingGzip() // 如果客服端接受 gzip 返回数据会以gzip压缩后返回，否则不压缩
