...
func main() {
	...
	subdomain := app.Party("*.") // 通配符子域名

	// subdomain := app.Party("admin.") // 固定子域名，测试未成功
	...
	app.Get("/", func(ctx iris.Context) {
		...
		if err := ctx.View("wildcard.html"); err != nil { // 通配符域名的 模板
			panic(err)
		}

		// paramsAsArray := []string{"theParam1", "theParam2", "paramThirdAfterStatic"}
		// ctx.ViewData("ParamsAsArray", paramsAsArray)
		// if err := ctx.View("subdomain.html"); err != nil { // 固定子域名的 模板
		// 	panic(err)
		// }
	})
