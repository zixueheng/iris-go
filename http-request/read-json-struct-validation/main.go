...
func main() {
	validate = validator.New() // 新建一个 验证器

	// Register validation for 'User'
	// NOTE: only have to register a non-pointer type for 'User', validator
	// internally dereferences during it's type checks.
	validate.RegisterStructValidation(UserStructLevelValidation, User{}) // 将 User 结构体注册到 验证器

	app := iris.New()
	app.Post("/user", func(ctx iris.Context) {
		var user User
		if err := ctx.ReadJSON(&user); err != nil { // 从请求体重读取Json放到参数指针指定的值当中
			// Handle error.
		}

		// Returns InvalidValidationError for bad validation input, nil or ValidationErrors ( []FieldError )
		// 对于无效的验证输入，返回 InvalidValidationError nil 或 ValidationErrors ( []FieldError )
