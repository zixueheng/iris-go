...
func main() {
	...
	app.Use(func(ctx iris.Context) { // 这个会在所有的分组路由和子路由 之前 起效（即是前置中间件），但不是 根路由 /
		println(`before the party's routes and its children, but this is not applied to the '/' route because it's registered before the middleware, order matters.`)
		ctx.Next()
	})
	...
	child.Get("/", func(ctx iris.Context) {
		ctx.Writef(`this is the localhost:8080/child route. All Use and Done handlers that are registered to the parent party, are applied here as well.`)
		ctx.Next() // call the done handlers.
	})
...
func before(ctx iris.Context) {
	...
	ctx.Next() // 执行下一个路由，这里指的是 mainHandler 函数
}
