...
// GetStartTime 获取的 注入的时间 是不变的
// http://localhost:8080/start/time
func (c *VisitController) GetStartTime() string {
	return c.StartTime.Format("2006-01-02 15:04:05")
}

// GetDynamicTime ...
// t 是动态注入的
// http://localhost:8080/dynamic/time
func (c *VisitController) GetDynamicTime(t string) string {
	return t
}
...
func newApp() *iris.Application {
	...
	visitApp.Register(
		...
		// 动态依赖 每次请求系统会 用请求的 context 作为参数调用这个函数
		func(ctx iris.Context) string {
			return time.Now().Format("2006-01-02 15:04:05") // 注意这里不能返回 time.Now()，因为下面的 有已个 同类型的参数，下面的会覆盖上面的
		},
