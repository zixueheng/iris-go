...
func main() {
	...
	m := mvc.New(app) // New(app) 相当于 New(app.Party("/"))
...
func (c *exampleController) AfterActivation(a mvc.AfterActivation) {
	...
	index := a.GetRoute("GetOther") // 根据 方法名 获取 方法的路由
	// just prepend the handler(s) as middleware(s) you want to use.
	// or append for "done" handlers.
	index.Handlers = append([]iris.Handler{cacheHandler}, index.Handlers...) // 将中间件 追加到 路由的 Handleers 字段中
}

// Get 没有添加 cacheHandler 中间件
func (c *exampleController) Get() string {
	now := time.Now().Format("Mon, Jan 02 2006 15:04:05")
	return "last time executed without cache: " + now
}

// GetOther 添加了 cacheHandler 中间件
