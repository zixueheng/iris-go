...
func main() {
	...
	// 将 session 注册到 app 的中间件中，这样 路由获取的 session 都是 同一个了（sessions.Get()）
	...
	app.Get("/set", func(ctx iris.Context) {
		...
		// 这里作者不建议 在 session 里 存储 大数据 如 切片 Map
	})
	...
	app.Get("/destroy", func(ctx iris.Context) {
		...
		// sess.Destroy(ctx) // 删除整个 session 包括客户端的 cookie
		// or
		sessions.Get(ctx).Destroy() // 这里只删除服务器端的 session
	})
	// Note about Destroy:
	//
	...
	app.Get("/set_immutable", func(ctx iris.Context) {
		...
		businessGet := session.Get("businessEdit").([]businessModel) // Get 返回的是 接口类型，所以使用 类型断言 .(type) 返回他实际 动态值
