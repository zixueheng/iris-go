...
// 这里演示 mydomain.com 跳转到 www.mydomain.com
...
func newApp() *iris.Application {
	...
	app.SubdomainRedirect(app, www) // 301 跳转到 www 子域名
...
func info(ctx iris.Context) {
	host := ctx.Host()                     // 域名 www.mydomain.com
	fullRequestURI := ctx.FullRequestURI() // 全路径 http://www.mydomain.com/users
	ctx.Writef("Host: %s, FullRequestURI: %s", host, fullRequestURI)

	method := ctx.Method()
	subdomain := ctx.Subdomain() // www
	path := ctx.Path()           // /users
	www := ctx.IsWWW()           // true

	ctx.Writef("\nIs www: %v \n\n", www)
	ctx.Writef("Method: %s, Subdomain: %s, Path: %s", method, subdomain, path)
}
